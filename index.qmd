...
investment in deliberate and informed design, distribution and
...
regard, the overwhelming focus remains on algorithmic work
...
limited compared to these requirements. For example, among the more
refined sources of text available are the 6.9M English Wikipedia
articles[^5] at an estimated 6.24B tokens. If we extend our reach to
other repositories, e.g. the approximately 85 million academic papers
available on Sci-hub[^6] or SCOPUS[^7], we gain a rough estimate of 700B
tokens. A similar figure might be estimated from libegen and the
7.5M[^8] books there. The internet archive has some 44M books and texts,
which may yield up to 4.4T tokens if we assume 'texts' have
approximately the same length as books. Thus, a more likely source for
the ever-increasing data requirements are repositories like Common
Crawl[^9]. But this too has limits, and we are projected to have too
...
[^6]: According to [sci-hub](https://www.sci-hub.se/about) on 9 May,
    2025.
...
[^9]: According to [Common Crawl](https://commoncrawl.org/) on 9 May,
    2025.
...
USD to train, including human labor, and hardware and energy. Cost for
...
of model training, and exceeding annual revenues of organizations like
OpenAI that are training the models [@kandpal2025position]. Thus, the
resources spent do not include the costs of producing training data, for
which lack of appropriate compensation has given rise to a number of
lawsuits (e.g. Authors Guild vs. OpenAI [^10]). In other words, the more
...
[^10]: https://authorsguild.org/app/uploads/2023/12/Authors-Guild-OpenAI-Microsoft-Class-Action-Complaint-Dec-2023.pdf
...
also costly. The field of AI metrics distinguishes between a) observable
AI system behavior from b) the targets of measurement - e.g. performance
on a benchmark from artificial intelligence - by treating them as
*computational constructs* - indirectly observable aspects of AI system
behavior [@gignac2024defining]. Thus the process that gives rise to the
data, which include the annotation interface and task instructions shown
to annotators, is also a measurement instrument [@beck2022improving].

The case study in this thesis contributes to the field of AI metrics by
...
accuracy [@birhane2022values] rather than careful measurement design or
long-term dataset stewardship [@liem2023treat]. Beyond the challenge of
gathering data at the necessary scale, are issues about the quality of
the data itself, specifically the quality of the overall process that
created the data. This challenge is essentially one of measurement
[@welty2019metrology].
...
is now practiced and reported - is overburdened and inefficient. A
primary resource is the labor of academic students, with one study in
2012 showing that one third of all academic output comes from PhD
students [@lariviere2012shoulders]. PhD students simultaneously show a
high prevalence of depression and anxiety - as high as 24%, and 17%
respectively in a recent meta-analysis [@satinsky2021systematic].

PhD students contribute substantially to the body of human knowledge via
academic publications [@rorstad2015publication]. In parallel, PhD
...
publications, and not theses, remain the key factor in the assessment of
their value as scientists [@anderson2022effect], with more and more
thesis content being comprised of academic publications
[@jackson2013completing].

Further, PhD students contribute to peer-review. Although in some venues
Early Career Researchers (ECRs; e.g. PhD students, but also Post
Doctoral Researchers) occurs with supervision\[\^21\], it has been shown
that ECRs perform a substantial amount of the work but do not receive
credit [@mcdowell2019co]. Further, ECRs, like PhD students are
completely responsible for their own reviews in high-volume conferences
like NeurIPS [@shah2018design].

Predictably, academics appear to be turning to LLMs for assistance, as
evidence of its use is showing in academic work in both peer reviews
[@liang2024monitoring], and in manuscripts [@gray2024chatgpt]. This
overburden raises questions beyond the poor evaluation of the models
that underlie 'AI' to the 'AI' research process itself, as well as to
its likelihood of applying improvements.
...
human inputs necessary to fine tune their models [^11]. Despite the rate
...
sexual, or hatespeech alone. Commonly used platforms like mTurk[^12],
and Prolific[^13] pay at or near minimum wage in the US. While the
...
[^11]: https://www.business-humanrights.org/pt/%C3%BAltimas-not%C3%ADcias/openai-and-sama-hired-underpaid-workers-in-kenia-to-filter-toxic-content-for-chatgpt/

[^12]: https://en.wikipedia.org/wiki/Amazon_Mechanical_Turk

[^13]: https://www.prolific.com/participants
...
The ever increasing volume of published manuscripts on AI and related
topics[^14] makes it impossible to stay abreast of the overall field:
...
2023[^15]. This makes it more and more difficult to find reviewers, and
...
[^14]: terms included AI, artificial intelligence, machine learning, and
    neural networks. See Appendix for specific search strings.

[^15]: https://github.com/tranhungnghiep/AI-Conference-Info?utm_source=chatgpt.com
...
studies [^16]. These replications are pre-registered, and may be
...
[^16]: [Collaborative Replications and Education
